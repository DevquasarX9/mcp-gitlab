...
| `gitlab_create_draft_note` | Create MR draft note | `project_id`, `merge_request_iid`, `note`, optional raw `position` | created draft note | Developer+ and write enabled | `POST /projects/:id/merge_requests/:iid/draft_notes` | safe-write | `gitlab_create_draft_note {"project_id":"group/project","merge_request_iid":7,"note":"Please add a test"}` | raw `position` is passed through to GitLab unchanged |
| `gitlab_update_draft_note` | Update MR draft note | `project_id`, `merge_request_iid`, `draft_note_id`, `note` or raw `position` | updated draft note | Developer+ and write enabled | `PUT /projects/:id/merge_requests/:iid/draft_notes/:draft_note_id` | safe-write | `gitlab_update_draft_note {"project_id":"group/project","merge_request_iid":7,"draft_note_id":12,"note":"Updated"}` | empty updates are blocked before calling GitLab |
| `gitlab_delete_draft_note` | Delete MR draft note | `project_id`, `merge_request_iid`, `draft_note_id` | deleted confirmation or GitLab response | Developer+ and write enabled | `DELETE /projects/:id/merge_requests/:iid/draft_notes/:draft_note_id` | safe-write | `gitlab_delete_draft_note {"project_id":"group/project","merge_request_iid":7,"draft_note_id":12}` | deletes an unpublished current-user draft |
| `gitlab_publish_draft_note` | Publish one MR draft note | `project_id`, `merge_request_iid`, `draft_note_id` | published note response | Developer+ and write enabled | `PUT /projects/:id/merge_requests/:iid/draft_notes/:draft_note_id/publish` | safe-write | `gitlab_publish_draft_note {"project_id":"group/project","merge_request_iid":7,"draft_note_id":12}` | publishes one pending draft note |
| `gitlab_bulk_publish_draft_notes` | Publish all MR draft notes | `project_id`, `merge_request_iid` | bulk publish response | Developer+ and write enabled | `POST /projects/:id/merge_requests/:iid/draft_notes/bulk_publish` | safe-write | `gitlab_bulk_publish_draft_notes {"project_id":"group/project","merge_request_iid":7}` | submits all current-user pending draft notes |
