...
| `gitlab_add_issue_comment` | Add issue note | `project_id`, `issue_iid`, `body` | created note | Guest+ and write enabled | `POST /projects/:id/issues/:iid/notes` | safe-write | `gitlab_add_issue_comment {"project_id":"group/project","issue_iid":42,"body":"Needs reproduction details"}` | note rate limits apply |
...
| `gitlab_add_merge_request_comment` | Add MR note | `project_id`, `merge_request_iid`, `body` | created note | Guest+ and write enabled | `POST /projects/:id/merge_requests/:iid/notes` | safe-write | `gitlab_add_merge_request_comment {"project_id":"group/project","merge_request_iid":7,"body":"Please add a test"}` | overview note, not diff thread |
